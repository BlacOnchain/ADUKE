...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div 
        className="relative w-full max-w-md bg-white border border-[#E8E6DD] rounded-3xl p-7 sm:p-8 shadow-2xl space-y-6 text-left"
      ...
      >
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="font-display text-2xl font-bold text-[#121110]">
              {mode === 'login' && 'Welcome to Àdùkẹ́'}
              {mode === 'signup' && 'Create Guest Account'}
              {mode === 'reset' && 'Password Recovery'}
            </span>
          </div>
          <p className="text-xs text-[#595852]">
            {mode === 'login' && 'Sign in to access your reservations, orders & dining perks.'}
            {mode === 'signup' && 'Join for expedited reservations, table allocations & order tracking.'}
